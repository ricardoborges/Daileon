...
class RiskFinding:
    def __init__(
        ...
        category: str, # "versioned_secret", "unignored_env", "cloud_credentials", "hardcoded_connection_string"
        title: str,
        description: str,
        recommendation: str,
...
def scan_repository_tree(tree: List[Dict[str, Any]]) -> List[RiskFinding]:
    """Escaneia a lista de caminhos de arquivos do repositório (Nível 1 - Tree Inspection).
    
    `tree` é uma lista de dicts retornada pelo GitLab API (ex: `[{'path': 'backend/.env', 'type': 'blob'}, ...]`).
    """
    ...
    for path in paths:
        ...
        # 1. Arquivos .env versionados (exceção para .env.example, .env.template, etc)
        ...
        # 4. Credenciais de Cloud e Infraestrutura
        ...
        # 6. Pastas de ambiente virtual Python commitadas
...
def scan_file_content(path: str, content: str) -> List[RiskFinding]:
    """Escaneia o conteúdo de arquivos de configuração específicos (Nível 2 - Content Inspection)."""
    findings: List[RiskFinding] = []
    if not content:
        return findings
    ...
    # A) .NET Core (appsettings.json, appsettings.*.json)
    ...
    # B) .NET Framework (web.config, app.config)
    ...
    # C) Node.js config (config/default.json, config/production.json, etc)
    ...
    # D) Python settings (local_settings.py, settings.py)
...
def _scan_appsettings_json(path: str, data: Any, findings: List[RiskFinding], current_prefix: str = ""):
    if isinstance(data, dict):
        for key, value in data.items():
            ...
            # Checar chaves de Secrets (JwtSecret, ApiKey, SendGridKey, Password, Secret, Token, etc)
